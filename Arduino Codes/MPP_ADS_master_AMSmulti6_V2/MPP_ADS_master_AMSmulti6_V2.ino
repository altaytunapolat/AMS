...
static const uint32_t PER_SLOT_GUARD_US = 1500;                                       
static const uint32_t SLOT_US           = FRAME_US + PER_SLOT_GUARD_US;               


static const uint16_t COLLECT_MARGIN_MS = 24;   
static const uint16_t IDLE_GAP_MS       = 5;    
static const uint16_t MAX_IDLE_WAIT_MS  = 50;   



static const float    ZERO_EPS          = 1.0e-6f;

static const uint8_t  ZERO_ACCEPT_RUNS  = 3;

static const float    UT_ABS_MAX        = 1.0e4f;
static const long     PHASE_ABS_MAX_US  = 5000;
...
static uint8_t zero_run[NUM_SLAVES + 1] = {0};      
static float   last_good_uT[NUM_SLAVES + 1] = {0};  
static bool    have_last_good[NUM_SLAVES + 1] = {false};




static bool filter_and_hold(uint8_t addr, float in_uT, int32_t in_phase, float &out_uT){
  if (addr < 1 || addr > NUM_SLAVES) return false;

  
  if (!isfinite(in_uT)) {
    if (have_last_good[addr]) { out_uT = last_good_uT[addr]; return true; } 
    else { out_uT = 0.0f; return true; } 
  }
  if (fabs(in_uT) > UT_ABS_MAX || labs(in_phase) > PHASE_ABS_MAX_US) {
    if (have_last_good[addr]) { out_uT = last_good_uT[addr]; return true; }
    else { out_uT = 0.0f; return true; }
  }

  
  bool is_zero = (fabs(in_uT) <= ZERO_EPS);

  if (is_zero){
    uint8_t zr = ++zero_run[addr];

    if (zr >= ZERO_ACCEPT_RUNS){
      
      out_uT = 0.0f;
      last_good_uT[addr]   = 0.0f;
      have_last_good[addr] = true;
      zero_run[addr]       = ZERO_ACCEPT_RUNS; 
      return true;
    } else {
      
      if (have_last_good[addr]) {
        out_uT = last_good_uT[addr];
        return true;
      } else {
        out_uT = 0.0f; 
        return true;
...
void setup(){
  ...
  delay(50);
  Serial.println("RS485 Master 6S (zero-deglitch + HOLD-LAST)");
}
...
void loop(){
    ...
    while(RS485_SERIAL.available()){
      ...
      if(r == 1){ 
        if(P.len == 12 && P.addr >= 1 && P.addr <= NUM_SLAVES){
          ...
          if(sq == seq){
            float out;
            if (filter_and_hold(P.addr, u, ph, out)) {
              s[P.addr].ok   = true;
              s[P.addr].uT   = out;      
  ...
  for(uint8_t i=1;i<=NUM_SLAVES;i++){
    ...
    if(s[i].ok){
      Serial.print(s[i].uT, 3);
      Serial.print("uT ph=");
      Serial.print(s[i].phase_err_us);
    } else {
      
      if (have_last_good[i]) {
        Serial.print(last_good_uT[i], 3);
        Serial.print("uT ph=HOLD");
      } else {
        
        Serial.print("0.000uT ph=INIT");
      }
    }
  }
  Serial.println();
